...
# Grid-cell size (degrees) for the accessibility-score heat map layer. ~0.006 degrees is
# roughly 650m at Melbourne's latitude -- small enough to show real texture, large enough
# that most cells still aggregate more than one venue. Matches
# notebooks/accessibility_score_heatmap.ipynb, which uses the same cell size.
HEATMAP_CELL_SIZE = 0.006
HEATMAP_MIN_VENUES_PER_CELL = 2
...
def export():
    ...
    for p in place_rows:
        ...
        # Share of the 4 tracked features confirmed "yes" -- the same simple, literal
        # definition used in notebooks/accessibility_score_heatmap.ipynb. Kept on the
        # place record too so the frontend doesn't need to recompute it from `features`.
        accessibility_score = sum(1 for ft in FEATURE_TYPES if features_out[ft]["value"] == "yes") / len(FEATURE_TYPES)
    ...
    # ---- accessibility-score heat map: aggregate venues into a lat/lng grid ----
    # Same method as notebooks/accessibility_score_heatmap.ipynb: floor each venue's
    # coordinates to a HEATMAP_CELL_SIZE-degree grid cell, average accessibilityScore
    # within each cell, and drop cells too sparse to trust (< HEATMAP_MIN_VENUES_PER_CELL
    # venues) -- a single report shouldn't paint an entire block one color.
    cells: dict[tuple[float, float], list[float]] = {}
    for p in places_out:
        cell_lat = math.floor(p["lat"] / HEATMAP_CELL_SIZE) * HEATMAP_CELL_SIZE
        cell_lng = math.floor(p["lng"] / HEATMAP_CELL_SIZE) * HEATMAP_CELL_SIZE
        cells.setdefault((round(cell_lat, 6), round(cell_lng, 6)), []).append(p["accessibilityScore"])

    heatmap_out = []
    for (cell_lat, cell_lng), scores in cells.items():
        if len(scores) < HEATMAP_MIN_VENUES_PER_CELL:
            continue
        heatmap_out.append({
            "latMin": cell_lat, "lngMin": cell_lng,
            "latMax": round(cell_lat + HEATMAP_CELL_SIZE, 6), "lngMax": round(cell_lng + HEATMAP_CELL_SIZE, 6),
            "meanScore": round(sum(scores) / len(scores), 4),
            "count": len(scores),
        })
    heatmap_out.sort(key=lambda c: (-c["count"], c["latMin"], c["lngMin"]))
    ...
    (OUT_DIR / "heatmap.json").write_text(json.dumps(heatmap_out, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
    ...
    print(f"Wrote {len(places_out)} places, {len(toilets_out)} toilets, "
          f"{len(tgsi_out)} TGSI points, {len(conflicts_out)} conflict candidates, "
          f"{len(heatmap_out)} heat map cells -> {OUT_DIR}")
